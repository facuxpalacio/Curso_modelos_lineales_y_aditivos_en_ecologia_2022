# IntroducciÃƒÂ³n a los modelos lineales
## DefiniciÃƒÂ³n de modelo lineal
```{r modelo lineal}
set.seed(999)
...
# CorrelaciÃƒÂ³n producto-momento de Pearson
cov(x, y)/(sd(x)*sd(y))
...
## Matrices de correlaciÃƒÂ³n
A continuaciÃƒÂ³n se presentan las medidas del tamanio corporal en adultos de tres especies de pinguinos en islas del ArchipiÃƒÂ©lago Palmer, AntÃƒÂ¡rtida. 
```{r mat corr}
...
## RegresiÃƒÂ³n lineal simple
$$y = beta_0 + beta_1 x + epsilon$$
$$epsilon sim N(0, sigma^2)$$

```{r regresiÃƒÂ³n simple}
x <- rnorm(100)
e <- rnorm(100, mean = 0, sd = 1)
y <- 2 + 1.5*x + e
plot(x, y)
...
# Coeficiente de determinaciÃƒÂ³n
...
## RelaciÃƒÂ³n entre regresiÃƒÂ³n y correlaciÃƒÂ³n
```{r reg vs corr}
...
## Matrices de grÃƒÂ¡ficos de dispersiÃƒÂ³n
...
## RegresiÃƒÂ³n lineal multiple
$$y = beta_0 + beta_1 x_1 + beta_2 x_2 + ... + beta_n x_n + epsilon$$
$$epsilon sim N(0, sigma^2)$$

Analizaremos la relaciÃƒÂ³n entre la longitud del pico (bill_length_mm), masa corporal (body_mass_g) y alto del pico (bill_depth_mm) en pinguinos Adelia ($Pygoscelis adeliae$).
...
# GrÃƒÂ¡fico de dispersiÃƒÂ³n con plano de regresiÃƒÂ³n
library(plot3D)
x <- Ad$body_mass_g
y <- Ad$bill_depth_mm
z <- Ad$bill_length_mm
...
## Variables categÃƒÂ³ricas (= *dummies*)
```{r dummies}
...
## Test de *t*
Se muestran los tiempos de coagulaciÃƒÂ³n sanguÃƒÂƒÃ‚Â­nea (min) en conejos bajo el efecto de dos drogas (A y B).
```{r test de t}
...
## AnÃƒÂ¡lisis de la varianza
Se quiere evaluar si hay diferencias entre la longitud del pico en las tres especies de pingÃƒÂ¼ino del ArchipiÃƒÂ©lago Palmer.
```{r anova}
...
- Calcule las medias y varianzas para cada especie, concentración y combinación de especie y concentración.
