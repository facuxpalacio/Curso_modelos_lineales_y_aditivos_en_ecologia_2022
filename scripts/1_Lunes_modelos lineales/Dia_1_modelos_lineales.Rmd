...
# DIA 1. Introduccion a los modelos lineales
...
sum.cuad.exp <- sum((mean(datos$y) - reg.simple$fitted)^2)
sum.cuad.tot <- sum((datos$y - mean(datos$y))^2)
R2 <- sum.cuad.exp/sum.cuad.tot
R2

hist(reg.simple$residuals)
plot(x, y)
...
### Relacion entre coeficientes de regresion y correlacion
```{r}
...
```
### Supuestos y transformaciones - COMPLETAR
### Colinealidad
```{r}
library(car)
vif(model.Ad)
...
### Variables categoricas - COMPLETAR
```{r}
...
### Test de t
```{r}
t_coag <- c(8.8, 8.4, 7.9, 8.7, 9.1, 9.6, NA, 
